...
namespace Scada.Web
{
	...
	public class MainData
	{
        ...
        private void RefrServerComm()
        {
            if (settFileName != "")
            {
                DateTime dateTime = ScadaUtils.GetLastWriteTime(settFileName);
        ...
        /// <summary>
        /// Получить права на объекты интерфейса, введённые в базу конфигурации, для заданного идентификатора роли
        /// </summary>
        public Dictionary<string, Right> GetRights(int roleID)
        {
            // обновление таблиц базы конфигурации при необходимости
            RefreshBase();

            Monitor.Enter(baseLock);
            Dictionary<string, Right> rightDict = new Dictionary<string, Right>();
            ...
            try
            {
                ...
                for (int i = 0; i < rowCnt; i++)
                {
                    ...
                    if (tblInterface.DefaultView.Count > 0)
                    {
                        ...
                        if (!rightDict.ContainsKey(name))
                            rightDict.Add(name, right);
                    }
                }
            }
            catch (Exception ex)
            {
                AppData.Log.WriteAction((Localization.UseRussian ? 
                    "Ошибка при получении прав на объекты интерфейса: " :
                    "Error getting rights for the interface objects: ") + ex.Message, Log.ActTypes.Exception);
            }
            finally
            {
                Monitor.Exit(baseLock);
