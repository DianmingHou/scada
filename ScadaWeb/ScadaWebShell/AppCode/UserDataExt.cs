 ...
 * Summary  : Application user data extension for the web shell
 * 
...
{
    /// <summary>
    /// Application user data extension for the web shell
    /// <para>Расширение данных пользователя для веб-оболочки</para>
    /// </summary>
    public static class UserDataExt
    {
        ...
        /// </summary>
        public const string DefaultStartPage = "~/About.aspx";
        ...
        /// <summary>
        /// Перейти на страницу входа в систему, если вход не выполнен
        /// </summary>
        public static void LoginIfNeeded(this UserData userData, HttpContext context, bool redirectToLogin = true)
        {
            if (!userData.LoggedOn)
            {
                // проверка входа пользователя, сохранённого в cookie
                if (userData.WebSettings.RemEnabled)
                {
                    string login;
                    bool loggedOn;
                ...
                if (!userData.LoggedOn && redirectToLogin)
                {
                    ...
                    context.Response.Redirect("~/Login.aspx?return=" + 
                        HttpUtility.UrlEncode(context.Request.Url.OriginalString));
                }
            }
        }
        ...
        /// </summary>
        public static void GoToStartPage(this UserData userData, HttpContext context, string startPage = "")
        {
            CheckContext(context);
            if (string.IsNullOrEmpty(startPage))
                startPage = DefaultStartPage;
            string forcedStartPage = context.Request.QueryString["return"];
            context.Response.Redirect(string.IsNullOrEmpty(forcedStartPage) ? startPage : forcedStartPage);
        }
